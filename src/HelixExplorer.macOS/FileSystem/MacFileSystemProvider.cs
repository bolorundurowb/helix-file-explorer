...
public sealed class MacFileSystemProvider(
    IShellFolderEnumerator shell,
    INetworkConnectionService networkConnections,
    ILogger<MacFileSystemProvider> logger)
    : IFileSystemProvider
{
    ...
    private static bool IsTrashPath(string path)
    {
        ...
        var trashPath = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile) + "/.Trash";
        return path.StartsWith(trashPath, StringComparison.OrdinalIgnoreCase);
    }
